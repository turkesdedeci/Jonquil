...
export default function CollectionPageClient({
  ...
  const filteredProducts = useMemo(() => {
    ...
    return result;
  }, [products, sortBy, filterType, filterColor, filterMaterial, filterSize]);
                ...
                >
                  ...
                  {hasActiveFilters && (
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#0f3f44] text-xs text-white">
                      {filterType.length + filterColor.length + filterMaterial.length + filterSize.length}
                    </span>
                  )}
                </button>
              </div>
                ...
                {filteredProducts.length === 0 ? (
                  ...
                  >
                    {filteredProducts.map((product, index) => {
                      ...
                      const badgeLabel = inStock ? getMeasurementLabel(product) : 'Yakında';
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between border-b border-[#e8e6e3] px-4 py-4 sm:px-6">
                <h3 id="collection-mobile-filters-title" className="text-base font-semibold text-[#1a1a1a] sm:text-lg">
                  Filtreler
                </h3>
                <button
                  onClick={() => setShowMobileFilters(false)}
